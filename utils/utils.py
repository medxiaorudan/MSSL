...
def get_output(output, task):
    ...
    #elif task in {'classify', 'SSL'}:
        #output = torch.max(output, dim=3)
    elif task == 'classify':
        output=torch.argmax(output.cuda(), dim=1)
#        output = torch.argmax(output["classify"].cuda(), dim=1)
    elif task == 'SSL_S':
        output = output.permute(0, 2, 3, 1)
        test, output = torch.max(output, dim=3)    
        print("max-test:",test.max())
        print("min-test:",test.min())
